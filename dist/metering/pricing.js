...
class CostCalculator {
    ...
    static estimate(model, inputTokens, outputTokens) {
        let pricing = exports.MODEL_PRICES[model];
        // Auto-fallback mapping for raw model names to "provider/model" format if they forget the prefix
        if (!pricing) {
            // Try guessing OpenAI prefix
            pricing = exports.MODEL_PRICES['openai/' + model];
        }
        if (!pricing) {
            // Try guessing Anthropic prefix
            pricing = exports.MODEL_PRICES['anthropic/' + model];
        }
        if (!pricing) {
            // Try guessing Google prefix
            pricing = exports.MODEL_PRICES['google/' + model];
        }
        if (!pricing) {
            // Try guessing DeepSeek prefix
            pricing = exports.MODEL_PRICES['deepseek/' + model];
        }
        if (!pricing) {
